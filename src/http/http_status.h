...
/*! class for HTTP status codes and reason phrases. */
class HttpStatus {
public:
  ...
  enum class Code {
    ...
    ContentTooLarge = 413, //!< Indicates that the server is refusing to process
    ...
    FailedDependency =
        424, //!< Means that the method could not be performed on the resource
             ...
             //!< that action failed.
    TooEarly = 425, //!< Indicates that the server is unwilling to risk
    ...
    NotExtended = 510, //!< \deprecated \parblock Obsoleted as the experiment
                       //!< has ended and there is no evidence of widespread
                       //!< use. \endparblock The policy for accessing the
                       //!< resource has not been met in the request. [RFC 2774]
    NetworkAuthenticationRequired =
        511, //!< Indicates that the client needs to
             //!< authenticate to gain network access.

    xxx_max = 1023
  };

  explicit HttpStatus(Code code) : code_(static_cast<int>(code)) {}
  explicit HttpStatus(int code) : code_(code) {}

  inline int toInt() const { return code_; }

  /*! Returns the standard HTTP reason phrase for a HTTP status code.
   * \param code An HTTP status code.
   * \return The standard HTTP reason phrase for the given \p code or an empty
   * \c std::string() if no standard phrase for the given \p code is known.
   */
